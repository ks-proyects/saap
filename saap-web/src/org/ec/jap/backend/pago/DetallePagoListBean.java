/**
 * 
 */
package org.ec.jap.backend.pago;

import java.util.HashMap;
import java.util.List;

import javax.annotation.PostConstruct;
import javax.ejb.EJB;
import javax.faces.bean.ManagedBean;
import javax.faces.bean.ViewScoped;
import javax.faces.event.ActionEvent;
import javax.faces.event.ValueChangeEvent;
import javax.faces.model.SelectItem;

import org.ec.jap.backend.pagina.Bean;
import org.ec.jap.backend.utilitario.Mensaje;
import org.ec.jap.bo.saap.DetallePlanillaBO;
import org.ec.jap.bo.saap.PeriodoPagoBO;
import org.ec.jap.entiti.saap.DetallePlanilla;
import org.ec.jap.enumerations.TipoListaValor;

/**
 * @author Freddy G Castillo C
 * 
 */
@ManagedBean
@ViewScoped
public class DetallePagoListBean extends Bean {

	@EJB
	PeriodoPagoBO periodoPagoBO;

	@EJB
	DetallePlanillaBO detallePlanillaBO;

	private Integer idPeriodopago;
	private List<SelectItem> periodos;

	private List<DetallePlanilla> detallePlanillas;

	public DetallePagoListBean() {
		super();
	}

	@PostConstruct
	public void init() {
		try {
			super.init();
			search(null);

		} catch (Exception e) {
			e.printStackTrace();
			displayMessage(e.getMessage(), Mensaje.SEVERITY_ERROR);
		}
	}

	public void search(ActionEvent event) {
		try {
			periodos = getSelectItems(getUsuarioCurrent(), null, "ListaValor.finAllPeridos", false, true, false, TipoListaValor.NQ, null);
			valueChange(null);
			redisplayAction(2, "MUL".equals(getParam3String()));
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			displayMessage(e.getMessage(), Mensaje.SEVERITY_ERROR);
		}

	}

	public void valueChange(ValueChangeEvent event) {
		try {
			idPeriodopago = event == null ? 0 : Integer.valueOf(event.getNewValue() != null ? event.getNewValue().toString() : "0");
			map = new HashMap<>();
			map.put("idPeriodoPago", idPeriodopago != null ? idPeriodopago : 0);
			map.put("tipoRegistro", getParam3String());
			map.put("idUsuario", getParam2Integer());
			detallePlanillas = detallePlanillaBO.findAllByNamedQuery("DetallePlanilla.findByUserAndTipoReg", map);
		} catch (Exception e) {
			e.printStackTrace();
			displayMessage(e.getMessage(), Mensaje.SEVERITY_ERROR);
		}

	}

	public Integer getIdPeriodopago() {
		return idPeriodopago;
	}

	public void setIdPeriodopago(Integer idPeriodopago) {
		this.idPeriodopago = idPeriodopago;
	}

	public List<DetallePlanilla> getDetallePlanillas() {
		return detallePlanillas;
	}

	public void setDetallePlanillas(List<DetallePlanilla> detallePlanillas) {
		this.detallePlanillas = detallePlanillas;
	}

	public List<SelectItem> getPeriodos() {
		return periodos;
	}

	public void setPeriodos(List<SelectItem> periodos) {
		this.periodos = periodos;
	}

}
