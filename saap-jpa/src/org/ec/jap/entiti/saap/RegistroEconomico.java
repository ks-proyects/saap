...
@NamedQueries({
		...
		@NamedQuery(name = "RegistroEconomico.finUser", query = "select DISTINCT u,cp, CASE WHEN EXISTS(select dp.idDetallePlanilla from DetallePlanilla dp WHERE  dp.idCabeceraPlanilla=cp AND dp.idRegistroEconomico=:idRegistroEconomico) THEN 'S' ELSE 'N' END,(select MAX(det.estado) FROM DetallePlanilla det WHERE det.idCabeceraPlanilla=cp AND det.idRegistroEconomico=:idRegistroEconomico),(select MAX(det) FROM DetallePlanilla det WHERE det.idCabeceraPlanilla=cp AND det.idRegistroEconomico=:idRegistroEconomico)  from CabeceraPlanilla cp inner join cp.idServicio l inner join l.idUsuario  u  WHERE ( UPPER(u.nombres) LIKE UPPER(CONCAT('%',:filtro,'%')) OR UPPER(u.apellidos) LIKE UPPER(CONCAT('%',:filtro,'%')) OR UPPER(u.cedula) LIKE UPPER(CONCAT('%',:filtro,'%')) ) AND   cp.idPeriodoPago IN (SELECT re.idPeriodoPago from RegistroEconomico re WHERE re.idRegistroEconomico=:idRegistroEconomico) AND u.estado='ACT' AND u.tipoUsuario='JAAP' ORDER BY u.nombres,u.apellidos"),
