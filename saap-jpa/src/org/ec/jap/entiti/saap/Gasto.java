...
@XmlRootElement
@NamedQueries({
		@NamedQuery(name = "Gasto.findAllGastosAcum", query = "SELECT SUM(g.valor) FROM Gasto g inner join g.idPeriodoPago pp WHERE  (pp.anio<=:anio or :anio=0) AND (pp.mes<=:mes or :mes=-1)"),
		@NamedQuery(name = "Gasto.findAllGastos", query = "SELECT SUM(g.valor) FROM Gasto g inner join g.idPeriodoPago pp WHERE  (pp.anio=:anio or :anio=0) AND (pp.mes=:mes or :mes=-1)"),
